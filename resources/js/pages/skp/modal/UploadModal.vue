...
<template>
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-transparent">
        <div class="w-full max-w-md rounded-lg bg-stone-700 bg-opacity-90 p-6 text-white">
            <h2 class="mb-4 rounded-sm border border-white p-2 text-center text-xl font-bold">{{ props.name }}</h2>
            <p class="mb-6 rounded-sm border border-white p-2">{{ props.title }}</p>
            ...
            <form @submit.prevent="submitForm">
                ...
                <div class="flex justify-end space-x-4">
                    ...
                    <button @click="cancel" class="rounded bg-red-500 px-4 py-2 text-white hover:bg-gray-300">Mboten</button>
                </div>
            </form>
        </div>
    </div>
